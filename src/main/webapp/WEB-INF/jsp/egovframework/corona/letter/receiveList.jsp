    	...
    	$(document).ready(function(){
			...
			$("#btnDelete").on("click", function(){
				 ...
				 if($('input:checkbox[name="chk"]:checked').length==0){
					...
					formObj.attr("action", "receiveDelete.do");
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                       <h1 class="mt-4"><i class="fas fa-envelope-open-text"></i> 받은쪽지함</h1>
...
<!-- 데이터 테이블-->
                               ...
                               <form name="myform" action="" method=post>
                                    ...
                                    <table class="table table-bordered"   width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                        	<c:forEach items="${letterList}" var = "lreceiver">
                                        		
                                          <tr>
                                     
                                          <td style="text-align: center"><input type="checkbox" name="chk" value="${lreceiver.lrno}" onchange="check2(this.form.checkall)" class="checkSelect"/> 
                                       </td>
                                          
                                      
								<td width="60px" nowrap style="text-align:center"><c:out value="${pageMaker.totalCount - lreceiver.rnum + 1}" /></td>
							
							 
							 <td style="text-align: center">
							
							<c:choose>
							 <c:when test="${service.readChk(lreceiver).getRflag() == 'N' }">
                                       		 <i style="font-size:25px; " class="far fa-envelope" title="읽지 않음"></i>
                                 </c:when>
                                 <c:otherwise>
                                       		 <i style="font-size:25px;color:#A4A4A4" class="far fa-envelope-open" title="읽음"></i>
                                 </c:otherwise>
...
<div class="dropdown">
  ...
  <div class="dropdown-content">
  	...
  	<c:when test="${adminService.anoCheck(lreceiver.writerno) != 0 }">
  		...
  		<a href="/lsend.do?writerno=${lreceiver.writerno}">쪽지보내기</a>
   </c:when>
  		 <c:otherwise>
  		 	<span style="text-decoration:line-through;color:color:#A4A4A4"><B>탈퇴한 회원(<c:out value="${lreceiver.writer}"/>)</B></span>
								...
								<td ><fmt:formatDate value="${lreceiver.regdate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
    					...
    					<script>
      						$(function(){
        						$('#searchBtn').click(function() {
          						self.location = "receiveList.do" + '${pageMaker.makeQuery(1)}' + "&searchType=" + 
    							...
    							<li class="page-item"><a class="page-link" href="receiveList.do${pageMaker.makeSearch(1)}"> <i class="fa fa-angle-double-left" style="font-size:20px;"></i></a></li>
    						</c:if>
    						<c:if test="${pageMaker.prev}">
    							<li class="page-item"><a class="page-link" href="receiveList.do${pageMaker.makeSearch(pageMaker.startPage - 1)}"> <i class="fa fa-angle-left" style="font-size:20px;"></i> </a></li>
    						</c:if>
    						...
    						<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    							...
    							<c:choose>
    								...
    								<li class="page-item-active" <c:out value="${pageMaker.cri.page == idx ? 'class=info' : '' }" />>
    									<a class="page-link" href="receiveList.do${pageMaker.makeSearch(idx)}"><B>${idx}</B></a></li>
    								</c:when>
    								<c:otherwise>
    								<li class="page-item" <c:out value="${pageMaker.cri.page == idx ? 'class=info' : '' }" />>
    									<a class="page-link" href="receiveList.do${pageMaker.makeSearch(idx)}">${idx}</a></li>
    								</c:otherwise>
    							</c:choose>
    						</c:forEach>
    						...
    						<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
    							<li class="page-item"><a class="page-link" href="receiveList.do${pageMaker.makeSearch(pageMaker.endPage + 1)}"> <i class="fa fa-angle-right" style="font-size:20px;"></i> </a></li>
    						 </c:if>
    						 <c:if test="${scri.page != pageMaker.totalPage}">
    							<li class="page-item"><a class="page-link" href="receiveList.do${pageMaker.makeSearch(pageMaker.totalPage)}"> <i class="fa fa-angle-double-right" style="font-size:20px;"></i> </a></li>
    						</c:if>
